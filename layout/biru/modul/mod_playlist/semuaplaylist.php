 ...
 <?php
  ...
  <b><span class style=\"color:#EA1C1C;\"> Playlist</span> Video</b> </div><br/>";
  ...
  $a = mysqli_query($conn,"SELECT jdl_playlist, playlist.id_playlist, gbr_playlist, playlist_seo,  
                  COUNT(video.id_video) as jumlah 
                  FROM playlist LEFT JOIN video 
                  ON playlist.id_playlist=video.id_playlist 
                  WHERE playlist.aktif='Y'  
				
                  GROUP BY jdl_playlist ASC LIMIT $posisi,$batas");
  ...
  <a href=playlist-$w[id_playlist]-$w[playlist_seo].html>
  <img  src='img_playlist/kecil_$w[gbr_playlist]' alt='$w[jdl_playlist]'  width=125 height=105
  class='tooltip'  title='<b>ada $w[jumlah] video</b>'/></a>
  <h5><p align='center'>$w[jdl_playlist]
  <br/><span class style=\"color:#EA1C1C;font-size:10px;\">($w[jumlah] video)</span></p></h5>
  </div>";}
  echo "</tr></table></div>";

  $jmldata     = mysqli_num_rows(mysqli_query($conn,"SELECT * FROM playlist"));
  ...
  <div class='post-title'>
  <b>Video<span class style=\"color:#EA1C1C;\"> Terbaru</span></b>
  </div>";      
  
  $terkini=mysqli_query($conn,"SELECT * FROM video ORDER BY id_video DESC LIMIT 5");
  
  while ($d2 = mysqli_fetch_array($terkini)){
  ...
  dilihat: $lihat pengunjung <span class style=\"color:#EA1C1C;\">|</span> komentar: $total_komentar</span> </div>
